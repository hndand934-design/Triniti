(() => {
  ...
  const SOUND_KEY = "triniti_mines_sound_v1";
  ...
  function formatMultiplierForLadder(m) {
  ...
  function renderLadder(minesCount, safeOpened) {
    ...
    for (let s = 1; s <= safeMax; s++) {
      ...
      const xTxt = `x${formatMultiplierForLadder(m)}`;
      const active = st && st.active && !st.over && safeOpened === s ? " active" : "";

      items.push(`
        <div class="lstep${active}">
          <div class="k">Шаг ${s}</div>
          <div class="x">${xTxt}</div>
        </div>
      `);
    }
  ...
  function clampMines() {
    ...
    if (st && st.active && !st.over) {
      minesRange.value = String(st.minesCount);
      if (minesView) minesView.textContent = String(st.minesCount);
      return;
    }
  ...
  function renderStats() {
    const minesCount = st && st.active ? st.minesCount : Math.floor(Number(minesRange?.value) || MIN_MINES);
    const safeMax = SIZE - minesCount;
  ...
  function startGame() {
    ...
    if (minesRange) minesRange.value = String(minesCount);
    if (minesView) minesView.textContent = String(minesCount);
  ...
  betMinus?.addEventListener("click", () => {
    if (!betInput || (st && st.active && !st.over)) return;
