(() => {
  ...
  const Wallet = (() => {
    ...
    if (!sw || typeof sw.getCoins !== "function" || typeof sw.setCoins !== "function" || typeof sw.addCoins !== "function") {
      // если shared кошелька нет — чтобы игра не падала
      let coins = 1000;
      return {
        get(){ return coins|0; },
  ...
  // Добавим надписи на лицах (чтобы точно было видно что выпало)
  function mountCoinLabels(){
    const front = coinEl.querySelector(".coinFace--front");
    const back = coinEl.querySelector(".coinFace--back");
    if (front && !front.querySelector(".labelText")) {
      const t = document.createElement("div");
      t.className = "labelText";
      t.textContent = "ОРЁЛ";
      front.appendChild(t);
    }
    if (back && !back.querySelector(".labelText")) {
      const t = document.createElement("div");
      t.className = "labelText";
      t.textContent = "РЕШКА";
      back.appendChild(t);
  ...
  function setPick(v) {
    ...
    resetRoundUI(); // важно: при смене выбора сбрасываем “результаты”
    beep(520, 50, 0.025);
  }
  ...
  function clampBet() {
    ...
    // как у тебя было: изменение ставки сбрасывает монету в purple + статусы
    resetRoundUI();
  }
  ...
  function playFlipAnim() {
    return new Promise((resolve) => {
      ...
      // гарантированный перезапуск анимации
      coinEl.classList.remove("spin");
