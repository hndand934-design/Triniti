(() => {
  ...
  // ---- crash RNG (скрытый краш-поинт) ----
  // Дает нормальное распределение, редко большие значения, но возможны.
  ...
  function setUI(){
    ...
    if (!inRound) {
      ...
      betHint.textContent = cashed ? `забрал x${fmt2(cashedAt)}` : "в полёте";
    } else {
      betHint.textContent = cashed ? `забрал x${fmt2(cashedAt)}` : "ставка сгорела";
    }
    ...
    if (state === STATE.WAIT) {
      ...
      overlayText.textContent = inRound ? (cashed ? "Ты забрал" : "Нажми «Забрать»") : "Ты не в раунде";
