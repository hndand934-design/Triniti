(() => {
  ...
  // open modal from any element with data-open="free"
  document.querySelectorAll('[data-open="free"]').forEach((el)=>{
  ...
  function drawWheel(){
    ...
    // ярче и разноцветнее (как реальное колесо)
    const colors = [
      "rgba(76,125,255,.35)",
      "rgba(255,86,186,.30)",
      "rgba(54,220,170,.28)",
  ...
  function spinToIndex(winIndex){
    return new Promise((resolve)=>{
      ...
      // центр сектора под стрелку сверху
      const ideal = -((winIndex + 0.5) * step);
      const fullTurns = 5 + rngInt(3); // 5..7 (красивее крутится)
