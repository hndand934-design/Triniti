(() => {
  ...
  function syncActiveByModal(name) {
    const topMap = {
      promo: '.navPill[data-open="promo"]',
      support: '.navPill[data-open="support"]',
      free: '.navPill[data-open="free"]'
    };

    const sideMap = {
      promo: '.sideItem[data-open="promo"]',
      support: '.sideItem[data-open="support"]',
      free: '.sideItem[data-open="free"]'
    };

    const toolbarMap = {
      support: '.toolbarTab[data-open="support"]',
      free: '.toolbarTab[data-open="free"]'
    };

    const bottomMap = {
      support: '.mobileBottomNav__item[data-open="support"]',
      free: '.mobileBottomNav__item[data-open="free"]'
    };

    const topEl = document.querySelector(topMap[name] || "");
    const sideEl = document.querySelector(sideMap[name] || "");
    const toolbarEl = document.querySelector(toolbarMap[name] || "");
    const bottomEl = document.querySelector(bottomMap[name] || "");

    if (topEl) setActiveInGroup(".navPill", topEl);
    if (sideEl) setActiveInGroup(".sideItem", sideEl);
    if (toolbarEl) setActiveInGroup(".toolbarTab", toolbarEl);
    if (bottomEl) setActiveInGroup(".mobileBottomNav__item", bottomEl);
  }
  ...
  function updateMenuStateByTarget(targetSelector) {
    ...
    if (targetSelector === "#gamesSec" || targetSelector === "#games") {
      const topGames = document.querySelector('.navPill[data-jump="#gamesSec"], .navPill[data-jump="#games"]');
  ...
  function initPromo() {
    $("promoApply")?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (msg) {
          msg.textContent = `Промокод активирован: +${reward} 🪙 ✅`;
        }
