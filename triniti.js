// triniti.js (ГОТОВЫЙ) — без изменений
(() => {
  ...
  $("promoApply")?.addEventListener("click", () => {
    ...
    if (!input) { if (msg) msg.textContent = "Введите промокод."; beep(240,80,0.03); return; }

    let used = {};
    try { used = JSON.parse(localStorage.getItem(PROMO_KEY_USED) || "{}"); } catch {}
    if (used[input]) { if (msg) msg.textContent = "Этот промокод уже использован."; beep(240,80,0.03); return; }

    const reward = PROMOS[input];
    if (!reward) { if (msg) msg.textContent = "Неверный промокод."; beep(240,80,0.03); return; }
