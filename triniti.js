(() => {
  // SharedWallet fallback (если shared/wallet.js не загрузился)
  ...
  function syncBalanceUI(){
    ...
    $("#sideBalance") && ($("#sideBalance").textContent = String(c));
  }
  ...
  let audioCtx = null;
  function getCtx(){ const AC = window.AudioContext||window.webkitAudioContext; if(!AC) return null; return audioCtx || (audioCtx=new AC()); }
  ...
  $("#soundBtn")?.addEventListener("click", async ()=>{
    ...
    if(soundOn && audioCtx && audioCtx.state==="suspended"){ try{ await audioCtx.resume(); }catch{} }
  });

  // Popolnit/Vyvesti (заглушки)
  const depositMock=()=>{ beep(520,55,0.02); alert("Пополнение отключено (виртуальные монеты)."); };
  const withdrawMock=()=>{ beep(240,70,0.03); alert("Вывод отключен (виртуальные монеты)."); };
  $("#depositBtn")?.addEventListener("click", depositMock);
  $("#withdrawBtn")?.addEventListener("click", withdrawMock);
  $("#depositBtn2")?.addEventListener("click", depositMock);
  $("#withdrawBtn2")?.addEventListener("click", withdrawMock);

  // Bonus +1000
  const bonus=()=>{ addCoins(1000); beep(760,70,0.03); };
  $("#bonusBtn")?.addEventListener("click", bonus);
  $("#bonusBtn2")?.addEventListener("click", bonus);

  // Modal Halyava
  const freeModal=$("#freeModal");
  const openModal=()=>{ freeModal?.classList.add("open"); freeModal?.setAttribute("aria-hidden","false"); beep(520,50,0.02); };
  const closeModal=()=>{ freeModal?.classList.remove("open"); freeModal?.setAttribute("aria-hidden","true"); beep(420,50,0.02); };

  $("#freeBtn")?.addEventListener("click", openModal);
  $("#freeBtn2")?.addEventListener("click", openModal);
  $("#freeBtnSide")?.addEventListener("click", openModal);
  ...
  // Tabs
  const tabDaily=$("#tabDaily"), tabSocial=$("#tabSocial");
  const paneDaily=$("#paneDaily"), paneSocial=$("#paneSocial");
  const setTab=(w)=>{
    const d=w==="daily";
    tabDaily?.classList.toggle("active",d);
    tabSocial?.classList.toggle("active",!d);
    paneDaily?.classList.toggle("hidden",!d);
    paneSocial?.classList.toggle("hidden",d);
    beep(520,45,0.02);
  };
  ...
  const rngInt=(n)=>{ const u=new Uint32Array(1); crypto.getRandomValues(u); return u[0]%n; };

  const getNextTs=()=>{ const v=Number(localStorage.getItem(DAILY_NEXT_KEY)||0); return Number.isFinite(v)?v:0; };
  const setNextTs24h=()=>{ const next=Date.now()+24*60*60*1000; localStorage.setItem(DAILY_NEXT_KEY,String(next)); return next; };
  ...
  const freeStatus=$("#freeStatus"), freeDot=$("#freeDot");
  ...
  function renderFree(){
    ...
    if(freeDot){
      freeDot.style.background=ok?"#26d47b":"#ff5a6a";
      freeDot.style.boxShadow=ok?"0 0 0 3px rgba(38,212,123,.14)":"0 0 0 3px rgba(255,90,106,.14)";
    }
  }
  ...
  function renderPrizes(){
    ...
    PRIZES.forEach(p=>{
      ...
      d.innerHTML=`<div class="p">${p.label}</div><div class="t">в кошелёк</div>`;
      prizeList.appendChild(d);
    });
  }
  ...
  function drawWheel(){
    ...
    // стартуем с -PI/2 (стрелка сверху) => синхрон 1в1
    for(let i=0;i<n;i++){
      ...
      ctx.fillStyle = (i%2===0) ? "rgba(109,75,255,.20)" : "rgba(59,124,255,.14)";
      ctx.fill();
  ...
  // КЛЮЧЕВОЕ: вращаемся так, чтобы ПОД СТРЕЛКОЙ был winIndex
  function spinToIndex(winIndex){
    return new Promise((resolve)=>{
      ...
      // хотим, чтобы он попал под стрелку (-PI/2) => R = -(i+0.5)*step (mod 2PI)
  ...
  async function onSpin(){
    ...
    const winIndex = rngInt(PRIZES.length);     // <- ИСТИНА
    await spinToIndex(winIndex);                // <- ВИЗУАЛ 1в1 С ИСТИНОЙ

    const prize = PRIZES[winIndex];
    addCoins(prize.coins);                      // <- НАЧИСЛЕНИЕ ТО ЖЕ
  ...
  // Social bonus
  const VK_KEY="triniti_bonus_vk_v1", TG_KEY="triniti_bonus_tg_v1";
  const claimVK=$("#claimVK"), claimTG=$("#claimTG");
  const vkState=$("#vkState"), tgState=$("#tgState");
