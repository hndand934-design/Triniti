(() => {
  ...
  $("promoApply")?.addEventListener("click", () => {
    ...
    if (!input) { msg && (msg.textContent = "Введите промокод."); beep(240,80,0.03); return; }

    const used = JSON.parse(localStorage.getItem(PROMO_KEY_USED) || "{}");
    if (used[input]) { msg && (msg.textContent = "Этот промокод уже использован."); beep(240,80,0.03); return; }

    const reward = PROMOS[input];
    if (!reward) { msg && (msg.textContent = "Неверный промокод."); beep(240,80,0.03); return; }

    used[input] = 1;
    localStorage.setItem(PROMO_KEY_USED, JSON.stringify(used));
    addCoins(reward);
    msg && (msg.textContent = `Промокод активирован: +${reward} 🪙 ✅`);
    beep(760,70,0.03); setTimeout(()=>beep(920,70,0.03), 90);
  });
  ...
  async function onSpin(){
    ...
    if(!canSpinNow()){
      renderTimer();
      dailyMsg && (dailyMsg.textContent="Пока нельзя. Дождись таймера 😉");
      beep(220,90,0.03);
      return;
    }
    ...
    wheelCenterTxt && (wheelCenterTxt.textContent="…");
    wheelCenterSub && (wheelCenterSub.textContent="крутим");
    dailyMsg && (dailyMsg.textContent="Крутим колесо…");
    beep(520,55,0.02);
    ...
    wheelCenterTxt && (wheelCenterTxt.textContent = `+${prize.coins}`);
    wheelCenterSub && (wheelCenterSub.textContent = "🪙 начислено");
    dailyMsg && (dailyMsg.textContent = `Выпало: ${prize.label} — начислено ✅`);
