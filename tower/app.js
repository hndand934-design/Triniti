(() => {
  ...
  function renderLadder() {
    ...
    for (let i = 0; i < ROWS; i++) {
      ...
      item.className = "ladderStep";
      item.innerHTML = `
        <div class="ladderStep__k">Шаг ${rowNum}</div>
        <div class="ladderStep__x">${fmtX(x)}</div>
      `;
  ...
  function updateLadderActive() {
    ...
    const items = Array.from(ladderEl.querySelectorAll(".ladderStep"));
    items.forEach((el) => el.classList.remove("active"));

    if (!state.inRound) return;

    if (items[state.currentRow]) {
      items[state.currentRow].classList.add("active");
    }
  }
