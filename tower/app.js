(() => {
  ...
  const LADDER = {
    normal: [1.18, 1.42, 1.72, 2.1, 2.6, 3.3, 4.2, 5.5],
    hard:   [1.35, 1.75, 2.35, 3.2, 4.2, 5.5, 7.2, 9.4]
  };
  ...
  function renderSoundUI() {
    if (soundText) {
      soundText.textContent = soundOn ? "Звук" : "Звук";
    }
  ...
  // ===== Round control =====
  function resetIdle(keepStatus = false) {
    state.inRound = false;
    state.busy = false;
    state.lost = false;
    state.cleared = 0;
